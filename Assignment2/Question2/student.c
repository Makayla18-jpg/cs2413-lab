...
int checkAVL(struct TreeNode* node, struct TreeNode* min, struct TreeNode* max) {
    if (node == NULL) {
        return 0;
    }
    ...
    int leftHeight = checkAVL(node->left, min, node);
    if (leftHeight == -1) return -1;

    int rightHeight = checkAVL(node->right, node, max);
